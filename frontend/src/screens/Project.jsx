...
const Project = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`/projects/get-project/${location.state._id}`).then(res => {
            ...
            <section className="left relative flex flex-col h-screen min-w-96 bg-slate-300">
                <header className="flex justify-between items-center p-2 px-4 w-full bg-slate-100 absolute top-0">
                    <button
                        ...
                        className="text-sm p-2 items-center flex rounded-full bg-slate-300">
                        <i className="ri-add-fill mr-1"></i>
                        <p>Add Collaborator</p>
                    </button>
                    <button
                        onClick={() => setIsSidePanelOpen(!isSidePanelOpen)}
                        className="text-sm p-2 rounded-full bg-slate-300">
                ...
                <div className="conversation-area pt-14 pb-10 flex-grow flex flex-col h-full relative">
                    ...
                    <div className="inputField w-full flex absolute bottom-0">
                        ...
                        <button
                            onClick={send}
                            className='px-4 bg-slate-800 text-white'><i className="ri-send-plane-fill"></i></button>
                    </div>
                </div>
                <div className={`sidePanel w-full h-full flex flex-col gap-2 bg-slate-50 absolute transition-all ${isSidePanelOpen ? 'translate-x-0' : '-translate-x-full'} top-0`}>
                    <header className="flex justify-between items-center p-2 px-3 bg-slate-200">
                        <p className="text-semibold">Collaborators</p>
                        <button
                            onClick={() => setIsSidePanelOpen(!isSidePanelOpen)}
                            className="p-2 px-3">
                    ...
                    <div className="file-tree w-full">
                        {Object.keys(fileTree).map((file, index) => (
                            <button
                                ...
                                className="tree-element cursor-pointer p-2 px-3 flex items-center gap-2 bg-slate-400 w-full">
                    ...
                    <div className="top flex justify-between w-full">
                        <div className="files flex">
                            {openFiles.map((file, index) => (
                                <button key={index} onClick={() => setCurrentFile(file)} className={`open-file cursor-pointer p-2 px-3 flex item-center gap-2 bg-slate-300 w-fit ${currentFile === file ? 'bg-slate-400' : ''} `}>
                                    <p className='font-semibold text-lg'>{file}</p>
                                </button>
                            ))}
                        </div>
                ...
                {iFrameUrl && webContainer && (
                    <div className="flex flex-col min-w-96 h-full">
                        <div className="address-bar">
                            <input type="text" onChange={() => setIFrameUrl(e.target.value)} value={iFrameUrl} className="w-full p-2 px-4 bg-slate-200" />
                        </div>
